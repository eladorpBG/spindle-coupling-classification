# Compare left and right channel XGBoost models within one cohort.
...
METRICS_FILE <- file.path(path.expand(OUTPUT_DIR), paste0(COHORT, "_3vs4_subject_metrics.rds"))
FIGURE_FILE <- file.path(path.expand(OUTPUT_DIR), paste0("combined_", COHORT, "_lateral_heatmaps.png"))
...
if (!all(c("3", "4") %in% data4elec$channel) ||
    any(!(data4elec$channel %in% c("3", "4")))) {
  stop("Expected left and right channels after preprocessing.")
}
